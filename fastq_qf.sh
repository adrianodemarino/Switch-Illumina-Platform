...
echo "
        ...
        +--------------------------------------------------------+
        |  Author:   Adriano De Marino, PhD student              |
        |  Date:     September 2019                              |
        |  Version:  2.1                                         |  
...
while getopts ":a:b:o:t:m:f:h" Option
	do
	case $Option in
		a ) R1=$OPTARG ;;
		b ) R2=$OPTARG ;;
...
printf "<`date +'%Y-%m-%d %H:%M:%S'`> Setup\n\n"
R1_NAME="`basename ${R1}`";
R2_NAME="`basename ${R2}`";

#**** ============================= METHOD: 1 ============================ ****#
if [ ${METHOD} = "slim" ]; then
	printf "<`date +'%Y-%m-%d %H:%M:%S'`> Creating R1 quality filtered reads list...\n"
	## remove low quality R1 reads
	trimmomatic SE -phred33 -threads $MAXTHREADS ${R1} "/dev/stdout" CROP:80 | fastq_quality_filter -q $FILTER -p 95 -Q 33 | fastq_to_fasta -Q 33 -n | fasta2csv | cut -d' ' -f1 > ${OUTDIR}/r1.quality.filtered.list
	printf "\n<`date +'%Y-%m-%d %H:%M:%S'`> Creating R2 quality filtered reads list...\n"
	## remove low quality R2 reads
	trimmomatic SE -phred33 -threads $MAXTHREADS ${R2} "/dev/stdout" CROP:12 | fastq_quality_filter -q $FILTER -p 100 -Q 33 | fastq_to_fasta -Q 33 -n | fasta2csv | cut -d' ' -f1 > ${OUTDIR}/r2.quality.filtered.list
	printf "\n<`date +'%Y-%m-%d %H:%M:%S'`> Intersecting R1 and R2 lists...\n\n"
	...
	printf "<`date +'%Y-%m-%d %H:%M:%S'`> Cleaning tmp files...\n\n"
	rm ${OUTDIR}/r1.quality.filtered.sorted.list ${OUTDIR}/r2.quality.filtered.sorted.list
	printf "<`date +'%Y-%m-%d %H:%M:%S'`> Filtering R1 and R2 FastQ files...\n"
...
if [ ${METHOD} = "meta" ]; then
	...
	## remove low quality R1 reads
	trimmomatic SE -phred33 -threads $MAXTHREADS ${R1} "/dev/stdout" CROP:80 | fastq_quality_filter -q $FILTER -p 95 -Q 33 | fastq_to_fasta -Q 33 -n | fasta2csv | cut -d' ' -f1 > ${OUTDIR}/r1.quality.filtered.list
	printf "\n<`date +'%Y-%m-%d %H:%M:%S'`> Filtering R1 and R2 FastQ files...\n"
